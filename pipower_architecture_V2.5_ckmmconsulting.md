...
| Version | Date | Notes |
|---------|------|-------|
| V2.5 | 2026-04-17 | BUGFIX: `pipower-shutdown.service` explicitly calls `pipower _pre-shutdown` via `ExecStartPre` before `systemctl poweroff`. Resolves ExecStop ordering issue when poweroff initiated from within a service. Shutdown sequence diagram updated. |
| V2.4 | 2026-04-17 | Added `log_event()` helper, `/var/log/pipower.log` persistent logging in `_pre-shutdown` and `_post-boot`, `pipower log` command, `TimeoutStopSec=30` on alarm service. File inventory and CLI table updated. |
...
pipower-shutdown.service starts
      ...
      │    - append ALARM_SET to /var/log/pipower.log
      │    (- prefix: poweroff proceeds even if this step fails)
      │
...
pipower-alarm.service stop triggered
      │  ExecStop: pipower _pre-shutdown   ← safety net for manual shutdowns
      │    (same logic — alarm already set by ExecStartPre for scheduled shutdowns)
      ▼
...
pipower-alarm.service starts
      │  ExecStart: pipower _post-boot
      │    - append BOOT to /var/log/pipower.log
      │    - check test_epoch, set tested=True if match
      ▼
...
| `log` | No | Display persistent shutdown/boot log (`--lines N`) |
